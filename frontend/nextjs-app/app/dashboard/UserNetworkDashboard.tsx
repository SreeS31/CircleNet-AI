...
import { addMemberToMyCircle, addMyRelationship, addPersonToMyNetwork, ApiError, createMyCircle, fetchMyCircles,
  ...
  fetchDirectMessages, sendDirectMessage, fetchDirectMessageAttachment, previewRelationshipBroadcast, sendRelationshipBroadcast, rankNetworkPeople } from '../lib/api';
import { startDirectCall, fetchIncomingCalls, fetchDirectCall, acceptDirectCall, rejectDirectCall, endDirectCall } from '../lib/api';
import type { BroadcastAudience, BroadcastAudienceType, CirclePost, CirclePostingPermission, DirectCall, DirectMessage, VisibilityScope } from '../lib/api';
...
function CircleAttachment({post}:{post:CirclePost}) {
  ...
  if(post.attachmentType?.startsWith('audio/'))return <audio className="circle-post-audio" src={url} controls preload="metadata"/>;
...
export default function UserNetworkDashboard({ username }: { username: string }) {
  ...
  const chooseBroadcastFile=(file?:File)=>{setBroadcastError('');if(!file){setBroadcastFile(undefined);return;}if(file.size>circleAttachmentLimit){setBroadcastError(`${file.name} is ${formatFileSize(file.size)}. Maximum attachment size is 25 MB.`);return;}if(!circleAttachmentTypes.has(file.type)){setBroadcastError(`${file.name} is not a supported file type.`);return;}setBroadcastFile(file);};
  const submitBroadcast=async()=>{setBroadcastError('');if(!broadcastAudience?.recipients.length){setBroadcastError('Preview the audience before sending.');return;}if(!broadcastDraft.trim()&&!broadcastFile){setBroadcastError('Write a message or choose an attachment before sending.');return;}setBusy(true);if(broadcastFile)setBroadcastProgress(0);try{const result=await sendRelationshipBroadcast(broadcastType,broadcastDraft,broadcastAnchor?Number(broadcastAnchor):undefined,broadcastLocation,broadcastFile,broadcastFile?setBroadcastProgress:undefined);setBroadcastDraft('');setBroadcastFile(undefined);setBroadcastAudience(null);setMessage(`Broadcast delivered privately to ${result.deliveredCount} ${result.deliveredCount===1?'person':'people'}${result.failedCount?`; ${result.failedCount} failed`:'.'}`);}catch(error){setBroadcastError(errorMessage(error));}finally{setBusy(false);setBroadcastProgress(undefined);}};
  ...
  return <main className="container user-network-dashboard">
    ...
    <section className={`network-section relationship-broadcast ${broadcastOpen?'is-open':''}`}><div className="broadcast-heading"><div><p className="eyebrow">PRIVATE BROADCAST</p><h2>Message a family level or area</h2><p>Each recipient receives a private one-to-one message; recipient details are never exposed.</p></div><button type="button" className="action-tag action-tag-admin" onClick={()=>setBroadcastOpen(value=>!value)}>{broadcastOpen?'Close':'New broadcast'}</button></div>{broadcastOpen&&<div className="broadcast-composer"><div className="broadcast-modes" role="radiogroup" aria-label="Broadcast audience"><label><input type="radio" checked={broadcastType==='HORIZONTAL'} onChange={()=>{setBroadcastType('HORIZONTAL');setBroadcastAudience(null);}}/><span>Same level</span><small>All children of a person</small></label><label><input type="radio" checked={broadcastType==='VERTICAL'} onChange={()=>{setBroadcastType('VERTICAL');setBroadcastAudience(null);}}/><span>Family branch</span><small>Person and descendants</small></label><label><input type="radio" checked={broadcastType==='LOCATION'} onChange={()=>{setBroadcastType('LOCATION');setBroadcastAudience(null);}}/><span>Area</span><small>Matching city, town, or area</small></label></div><div className="broadcast-target">{broadcastType==='LOCATION'?<input value={broadcastLocation} onChange={event=>{setBroadcastLocation(event.target.value);setBroadcastAudience(null);}} placeholder="City, town, or area"/>:<select value={broadcastAnchor} onChange={event=>{setBroadcastAnchor(event.target.value);setBroadcastAudience(null);}}><option value="">Choose a family member</option>{relationships.filter((item,index,all)=>all.findIndex(candidate=>candidate.person.id===item.person.id)===index).map(item=><option key={item.person.id} value={item.person.id}>{item.person.displayName}</option>)}</select>}<button type="button" className="action-tag action-tag-admin" disabled={busy} onClick={()=>void previewBroadcast()}>Preview audience</button></div>{broadcastAudience&&<div className="broadcast-preview"><strong>{broadcastAudience.recipients.length} recipients</strong><div>{broadcastAudience.recipients.map(person=><span key={person.userId}><PersonAvatar name={person.displayName} photo={person.profilePhoto}/>{person.displayName}</span>)}</div>{broadcastAudience.excludedCount>0&&<small>{broadcastAudience.excludedCount} managed or inactive profiles excluded.</small>}</div>}<textarea value={broadcastDraft} onChange={event=>setBroadcastDraft(event.target.value)} placeholder="Write one private message for this audience…" maxLength={4000}/><div className="broadcast-actions"><label className="action-tag action-tag-admin circle-file-button"><input type="file" onChange={event=>{chooseBroadcastFile(event.target.files?.[0]);event.currentTarget.value='';}}/>Attach file</label>{broadcastFile&&<span>{broadcastFile.name} · {formatFileSize(broadcastFile.size)}</span>}{broadcastProgress!==undefined&&<span className="circle-upload-progress" style={{background:`conic-gradient(#705bd1 ${broadcastProgress*3.6}deg,#e8e2f4 0deg)`}}><strong>{broadcastProgress}%</strong></span>}<button type="button" className="btn btn-primary" disabled={busy||!broadcastAudience?.recipients.length} onClick={()=>void submitBroadcast()}>Send privately</button></div>{broadcastError&&<p className="circle-composer-error" role="alert">{broadcastError}</p>}</div>}</section>
