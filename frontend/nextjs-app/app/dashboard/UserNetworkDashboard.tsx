...
export default function UserNetworkDashboard({ username }: { username: string }) {
  ...
  const relationshipNode = (item: NetworkRelationship, paired = false) => <article className={`relationship-node ${paired ? 'spouse-node' : ''}`} key={item.id}>
    <div className="relationship-node-main"><span className="person-avatar">{item.person.displayName.charAt(0)}</span><div className="relationship-identity"><strong>{item.person.displayName}</strong><p>{item.person.phoneNumber}</p><div><span className="relationship-badge">{item.type}</span><span className={`status-tag ${item.person.accountStatus === 'INVITED' ? 'status-not-verified' : 'status-verified'}`}>{item.person.accountStatus === 'INVITED' ? 'Not Verified' : 'Verified'}</span></div></div><button className="action-tag action-tag-danger" onClick={async () => { await removeMyRelationship(item.id); await refresh(); }}>Remove</button></div>
