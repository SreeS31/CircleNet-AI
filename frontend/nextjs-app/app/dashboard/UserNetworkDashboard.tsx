...
function SearchableSelect({ value, placeholder, options, onChange, className = '' }: { value: string; placeholder: string; options: string[]; onChange: (value: string) => void; className?: string }) {
  ...
  return <details className={`searchable-select ${className}`}>
    ...
    <div className="searchable-select-menu">
      <input type="search" value={query} onChange={event => setQuery(event.target.value)} placeholder={`Search ${placeholder.toLowerCase()}â€¦`} aria-label={`Search ${placeholder.toLowerCase()}`} />
      <div>{filtered.map(option => <button type="button" className={option === value ? 'selected' : ''} key={option} onClick={event => { onChange(option); setQuery(''); event.currentTarget.closest('details')?.removeAttribute('open'); }}><span>{option}</span>{option === value && <strong>âœ“</strong>}</button>)}{!filtered.length && <p>No matching options</p>}</div>
    </div>
  </details>;
}
...
export default function UserNetworkDashboard({ username }: { username: string }) {
  ...
  const addByMobile = async (event: FormEvent) => {
    ...
    try {
      ...
      setMessage(identityType === 'MANAGED'
        ...
        : existing
        ? `${relationship.person.displayName} already exists. Only the ${directRelationshipType.toLowerCase()} relationship was addedâ€”no duplicate user was created.`
  ...
  const relationshipNode = (item: NetworkRelationship, paired = false) => {
    ...
    <div className="relationship-node-main"><PersonAvatar name={item.person.displayName} photo={item.person.profilePhoto}/><div className="relationship-identity"><strong>{item.person.displayName}</strong><div className="private-contact-display">{item.contactPhone && <span><i aria-hidden="true">â˜Ž</i>{item.contactPhone}</span>}{item.contactEmail && <span><i aria-hidden="true">âœ‰</i>{item.contactEmail}</span>}</div><div><span className="relationship-badge">{item.type}</span><span className="status-tag status-view">{visibilityOptions.find(option => option.value === item.visibilityScope)?.label || 'Friends'}{item.visibilityCompany ? ` Â· ${item.visibilityCompany}` : ''}</span><PersonStatus person={item.person}/></div></div><div className="relationship-actions"><button className="action-tag action-tag-admin" onClick={() => setEditingRelationship({ id:item.id, contactName:item.person.displayName, contactPhone:item.contactPhone || '', contactEmail:item.contactEmail || '', type:item.type, visibilityScope:item.visibilityScope || 'FRIENDS', visibilityCompany:item.visibilityCompany || '' })}>Edit</button><button className="action-tag action-tag-danger" onClick={async () => { await removeMyRelationship(item.id); await refresh(); }}>Remove</button></div></div>
    ...
    <details className="circle-picker"><summary className={`action-tag action-tag-admin ${!administeredCircles.length || allCirclesContainPerson ? 'disabled' : ''}`}>{allCirclesContainPerson ? 'âœ“ In all circles' : 'Add to circle'}</summary><div className="circle-picker-menu"><input className="circle-picker-search" type="search" value={circleSearch[item.id] || ''} onChange={e => setCircleSearch({...circleSearch,[item.id]:e.target.value})} placeholder="Search circlesâ€¦" aria-label="Search circles"/>{filteredCircles.map(circle => { const alreadyAdded = circle.members.some(member => member.person.id === item.person.id); return <button type="button" className={alreadyAdded ? 'circle-already-added' : ''} key={circle.id} disabled={busy || alreadyAdded} onClick={event => { void addToCircle(item.person, circle.id); event.currentTarget.closest('details')?.removeAttribute('open'); }}><span>{circle.name}</span>{alreadyAdded && <strong aria-label="Already in this circle">âœ“ Added</strong>}</button>; })}{!administeredCircles.length && <span>No circles you administer</span>}{administeredCircles.length > 0 && filteredCircles.length === 0 && <span>No matching circles</span>}</div></details>
  </article>;
  };
  ...
  return <main className="container user-network-dashboard">
    ...
    <section className="card quick-add-card">
      ...
      <form onSubmit={addByMobile} className="quick-add-form">
        <SearchableSelect className="quick-add-identity" value={identityType === 'ACCOUNT' ? 'CircleNet account' : 'Managed person'} placeholder="Person type" options={['CircleNet account', 'Managed person']} onChange={value => setIdentityType(value === 'Managed person' ? 'MANAGED' : 'ACCOUNT')}/>
        ...
        <input className="quick-add-mobile" type="tel" required={identityType === 'ACCOUNT'} value={mobileToAdd} onChange={e => setMobileToAdd(e.target.value)} placeholder={identityType === 'ACCOUNT' ? 'Mobile number, e.g. +919876543210' : 'Mobile number (optional)'} />
        ...
        <button className="btn btn-primary" disabled={busy}>{busy ? 'Checkingâ€¦' : 'Add person'}</button>
      </form>
    ...
    <section className="network-layout">
      <article className="card network-search-card">
        ...
        <form className="network-search" onSubmit={search}>
          <input required value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Person name, surname, mobile number, or locationâ€¦" />
