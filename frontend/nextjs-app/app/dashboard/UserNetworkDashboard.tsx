...
export default function UserNetworkDashboard({ username }: { username: string }) {
  ...
  const relationshipNode = (item: NetworkRelationship, paired = false) => <article className={`relationship-node ${paired ? 'spouse-node' : ''}`} key={item.id}>
    ...
    <div className="relationship-circle-controls"><select value={circleChoice[item.person.id] || ''} onChange={e => setCircleChoice({...circleChoice, [item.person.id]: e.target.value})}><option value="">Choose circle</option>{circles.map(circle => <option value={circle.id} key={circle.id}>{circle.name}</option>)}</select><button className="btn btn-secondary" disabled={busy || !circles.length} onClick={() => addToCircle(item.person)}>Add to circle</button></div>
  </article>;
  ...
  return <main className="container user-network-dashboard">
    ...
    <section className="network-section relationship-tree-section"><div className="section-heading"><div><p className="eyebrow">MY RELATIONSHIP MAP</p><h2>My relationships</h2></div><span>{relationships.length}</span></div>{relationships.length ? <div className="relationship-tree"><div className={`partnership-row ${spouseRelationships.length ? 'has-spouse' : ''}`}><article className="self-node"><span className="self-avatar">{username.charAt(0).toUpperCase()}</span><div><strong>{username}</strong><small>You</small></div></article>{spouseRelationships.map(item => <div className="spouse-pair" key={item.id}><span className="partner-connector"><i>♥</i></span>{relationshipNode(item, true)}</div>)}</div>{otherRelationships.length > 0 && <div className="relationship-branches">{otherRelationships.map(item => <div className="relationship-branch" key={item.id}><span className="relationship-branch-line" />{relationshipNode(item)}</div>)}</div>}</div> : <p className="circle-empty-state">Add someone above to start your relationship tree.</p>}</section>

    <section className="network-section circle-tree-section"><div className="section-heading"><div><p className="eyebrow">MY NETWORK MAP</p><h2>My circles</h2></div><span>{circles.length}</span></div><div className="circle-forest">{circles.map(circle => <article className="circle-tree" key={circle.id}><div className="circle-root"><span className="circle-root-icon">◎</span><div><h3>{circle.name}</h3><p>{circle.description || 'Your private circle'}</p><small>{circle.members.length} {circle.members.length === 1 ? 'member' : 'members'}</small></div></div><div className={`circle-branches ${circle.members.length ? '' : 'circle-branches-empty'}`}>{circle.members.map(member => <div className="circle-branch" key={member.id}><span className="branch-line" /><div className="circle-member-node"><span className="person-avatar">{member.displayName.charAt(0)}</span><div><strong>{member.displayName}</strong><small>{member.accountStatus === 'INVITED' ? 'Invitation pending' : 'CircleNet member'}</small></div><button className="text-button danger" onClick={async () => { await removeMemberFromMyCircle(circle.id, member.id); await refresh(); }}>Remove</button></div></div>)}{!circle.members.length && <div className="empty-branch"><span>＋</span><p>Add relationships using the controls above</p></div>}</div></article>)}</div>{!circles.length && <p className="circle-empty-state">Create your first circle to start a visual relationship map.</p>}</section>
  </main>;
}
