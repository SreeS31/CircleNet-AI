...
import { addMemberToMyCircle, addMyRelationship, addPersonToMyNetwork, ApiError, createMyCircle, fetchMyCircles,
  ...
  createCirclePost, fetchCircleAttachment, fetchCirclePosts, demoteCircleAdmin, promoteCircleAdmin, removeMemberFromMyCircle, removeMyRelationship, searchNetworkPeople, updateMyRelationship, updateMyCircle } from '../lib/api';
import type { CirclePost, CirclePostingPermission, VisibilityScope } from '../lib/api';
...
export default function UserNetworkDashboard({ username }: { username: string }) {
  ...
  const sendCirclePost=async(circleId:number)=>{const draft=circleDrafts[circleId]||'';const file=circleFiles[circleId];if(!draft.trim()&&!file){setMessage('Write a message or choose a file.');return;}setBusy(true);try{await createCirclePost(circleId,draft,file,replyingTo[circleId]?.id);setCircleDrafts(current=>({...current,[circleId]:''}));setCircleFiles(current=>({...current,[circleId]:undefined}));setReplyingTo(current=>({...current,[circleId]:undefined}));const loaded=await fetchCirclePosts(circleId);setCirclePosts(current=>({...current,[circleId]:loaded}));setMessage('Message posted to the circle.');}catch(error){setMessage(errorMessage(error));}finally{setBusy(false);}};
