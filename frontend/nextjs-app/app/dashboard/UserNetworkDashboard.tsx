...
export default function UserNetworkDashboard({ username }: { username: string }) {
  ...
  const relationshipNode = (item: NetworkRelationship, paired = false) => <article className={`relationship-node ${paired ? 'spouse-node' : ''}`} key={item.id}>
    <div className="relationship-node-main"><span className="person-avatar">{item.person.displayName.charAt(0)}</span><div className="relationship-identity"><strong>{item.person.displayName}</strong><p>{item.person.phoneNumber}</p><div><span className="relationship-badge">{item.type}</span>{item.person.accountStatus === 'INVITED' && <span className="pending-badge">Invitation pending</span>}</div></div><button className="text-button danger" onClick={async () => { await removeMyRelationship(item.id); await refresh(); }}>Remove</button></div>
    <details className="circle-picker"><summary className={`btn btn-secondary ${!ownedCircles.length ? 'disabled' : ''}`}>Add to circle</summary><div className="circle-picker-menu">{ownedCircles.map(circle => <button type="button" key={circle.id} disabled={busy} onClick={event => { void addToCircle(item.person, circle.id); event.currentTarget.closest('details')?.removeAttribute('open'); }}>{circle.name}</button>)}{!ownedCircles.length && <span>Create a circle first</span>}</div></details>
  </article>;
  ...
  return <main className="container user-network-dashboard">
        ...
        <div className="search-results">
          {results.map(person => {
            ...
            return <div className="people-result" key={person.id}>
              ...
              </div> : <div className="people-controls"><span className="relationship-badge">{relationship.type}</span><select value={circleChoice[person.id] || ''} onChange={e => setCircleChoice({...circleChoice, [person.id]: e.target.value})}><option value="">Choose circle</option>{ownedCircles.map(circle => <option value={circle.id} key={circle.id}>{circle.name}</option>)}</select><button className="btn btn-secondary" disabled={busy || !ownedCircles.length} onClick={() => addToCircle(person)}>Add to circle</button></div>}
            </div>;
          })}
        </div>
      </article>
    ...
    <section className="network-section relationship-tree-section"><div className="section-heading"><div><p className="eyebrow">MY RELATIONSHIP MAP</p><h2>My relationships</h2></div><span>{relationships.length}</span></div>{relationships.length ? <div className="relationship-tree"><div className={`partnership-row ${spouseRelationships.length ? 'has-spouse' : ''}`}><article className="self-node"><span className="self-avatar">{username.charAt(0).toUpperCase()}</span><div><strong>{username}</strong><small>You</small></div></article>{spouseRelationships.map(item => <div className="spouse-pair" key={item.id}><span className="partner-connector"><i>♥</i></span>{relationshipNode(item, true)}</div>)}</div>{otherRelationships.length > 0 && <div className={`relationship-branches ${spouseRelationships.length ? 'from-paired-self' : ''}`}>{otherRelationships.map(item => <div className="relationship-branch" key={item.id}><span className="relationship-branch-line" />{relationshipNode(item)}</div>)}</div>}</div> : <p className="circle-empty-state">Add someone above to start your relationship tree.</p>}</section>

    <section className="network-section circle-tree-section"><div className="section-heading"><div><p className="eyebrow">MY NETWORK MAP</p><h2>My circles</h2><p className="circle-summary">{ownedCircles.length} created by me · {circles.length - ownedCircles.length} added by others</p></div><span>{circles.length}</span></div><div className="circle-forest">{circles.map(circle => <article className="circle-tree" key={circle.id}><div className="circle-root"><span className="circle-root-icon">◎</span><div><h3>{circle.name}</h3><p>{circle.description || 'Private circle'}</p><small>{circle.ownedByCurrentUser ? 'Created by you' : `Added by ${circle.ownerName}`} · {circle.members.length} {circle.members.length === 1 ? 'member' : 'members'}</small></div></div><div className={`circle-branches ${circle.members.length ? '' : 'circle-branches-empty'}`}>{circle.members.map(member => <div className="circle-branch" key={member.id}><span className="branch-line" /><div className="circle-member-node"><span className="person-avatar">{member.displayName.charAt(0)}</span><div><strong>{member.displayName}</strong><small>{member.accountStatus === 'INVITED' ? 'Invitation pending' : 'CircleNet member'}</small></div>{circle.ownedByCurrentUser && <button className="text-button danger" onClick={async () => { await removeMemberFromMyCircle(circle.id, member.id); await refresh(); }}>Remove</button>}</div></div>)}{!circle.members.length && <div className="empty-branch"><span>＋</span><p>{circle.ownedByCurrentUser ? 'Add relationships using the controls above' : 'No members yet'}</p></div>}</div></article>)}</div>{!circles.length && <p className="circle-empty-state">You have not created or joined any circles yet.</p>}</section>
  </main>;
}
