...
export default function UserNetworkDashboard({ username }: { username: string }) {
  ...
  const relationshipNode = (item: NetworkRelationship, paired = false) => <article className={`relationship-node ${paired ? 'spouse-node' : ''}`} key={item.id}>
    ...
    <details className="circle-picker"><summary className={`btn btn-secondary ${!administeredCircles.length ? 'disabled' : ''}`}>Add to circle</summary><div className="circle-picker-menu">{administeredCircles.map(circle => <button type="button" key={circle.id} disabled={busy} onClick={event => { void addToCircle(item.person, circle.id); event.currentTarget.closest('details')?.removeAttribute('open'); }}>{circle.name}</button>)}{!administeredCircles.length && <span>No circles you administer</span>}</div></details>
  </article>;
  ...
  return <main className="container user-network-dashboard">
    ...
    <section className="network-section circle-tree-section"><div className="section-heading"><div><p className="eyebrow">MY NETWORK MAP</p><h2>My circles</h2><p className="circle-summary">{ownedCircles.length} created by me · {circles.length - ownedCircles.length} added by others</p></div><span>{circles.length}</span></div><div className="circle-forest">{circles.map(circle => <article className="circle-tree" key={circle.id}><div className="circle-root"><span className="circle-root-icon">◎</span><div><h3>{circle.name}</h3><p>{circle.description || 'Private circle'}</p><small>{circle.ownedByCurrentUser ? 'Created by you' : `Created by ${circle.ownerName}`} · {circle.members.length} {circle.members.length === 1 ? 'member' : 'members'}{circle.currentUserAdmin ? ' · You are an admin' : ''}</small></div></div><div className={`circle-branches ${circle.members.length ? '' : 'circle-branches-empty'}`}>{circle.members.map(member => <div className="circle-branch" key={member.person.id}><span className="branch-line" /><div className="circle-member-node"><span className="person-avatar">{member.person.displayName.charAt(0)}</span><div><strong>{member.person.displayName}</strong><small>{member.creator ? 'Creator · Admin' : member.admin ? 'Circle admin' : member.person.accountStatus === 'INVITED' ? 'Invitation pending' : 'CircleNet member'}</small></div>{circle.currentUserAdmin && !member.creator && <div className="member-admin-actions"><button className="text-button" onClick={async () => { if (member.admin) await demoteCircleAdmin(circle.id, member.person.id); else await promoteCircleAdmin(circle.id, member.person.id); await refresh(); }}>{member.admin ? 'Remove admin' : 'Make admin'}</button><button className="text-button danger" onClick={async () => { await removeMemberFromMyCircle(circle.id, member.person.id); await refresh(); }}>Remove</button></div>}</div></div>)}</div></article>)}</div>{!circles.length && <p className="circle-empty-state">You have not created or joined any circles yet.</p>}</section>
  </main>;
}
