...
export default function UserNetworkDashboard({ username }: { username: string }) {
  ...
  return <main className="container user-network-dashboard">
    <header className="network-header">
      ...
      <div className="network-actions"><button type="button" className="btn btn-secondary" onClick={()=>openBroadcast('FAMILY')}>Family broadcast</button><button type="button" className="btn btn-secondary" onClick={()=>openBroadcast('AREA')}>Area broadcast</button><Link href="/profile" className="btn btn-secondary">My profile</Link><Link href="/session" className="btn btn-secondary">Session</Link><button className="btn btn-secondary" onClick={async () => { await logout(); router.replace('/auth'); }}>Sign out</button></div>
    </header>
